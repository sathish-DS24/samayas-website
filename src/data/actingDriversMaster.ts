...
export interface DistrictDriverItem {
  id: string;
  district: string;
  state: string;
  slug: string;
...
export const actingDriversDataset: DistrictDriverItem[] = districts.map((d) => {
...
export function getActingDriverBySlug(slug?: string): DistrictDriverItem | null {
  if (!slug) return null;
