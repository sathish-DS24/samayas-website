export interface AirportRouteFare {
  hatchback: number;
  sedan: number;
  suv: number;
  innova: number;
...
export interface AirportRouteItem {
  ...
  airportIata: string;
  destination: string;
  direction: 'AirportToCity' | 'CityToAirport';
  distanceKm: number;
  travelTime: string;
  vehicleFare: AirportRouteFare;
  tollEstimate: string;
  ...
  highwayInfo: string[];
  exploreAlongRoute?: any;
  relatedRoutes: { name: string; slug: string; distance: string; startingFare: number }[];
...
function computeAirportFare(distanceKm: number): AirportRouteFare {
...
export const airportRoutesDataset: AirportRouteItem[] = [
...
export function getAirportRouteBySlug(slug?: string): AirportRouteItem | null {
  if (!slug) return null;
  const normalized = slug.toLowerCase().trim();
  return airportRoutesDataset.find((r) => r.slug.toLowerCase() === normalized) || null;
}

export function getAirportRoutesByAirport(airportSlug?: string): AirportRouteItem[] {
